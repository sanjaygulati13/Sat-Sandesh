...
//                        System.out.println("select count(s.asn) from subdetails s, basic b where (s.state like '"+state+"%' and b.subnos not in ('NA')");
			c1.rs=c1.st.executeQuery("select count(s.asn) from subdetails s, basic b where (s.state like '"+state+"%' and b.subnos not in ('NA') and s.asn=b.asn)");
			while(c1.rs.next())
			{
				x=c1.rs.getInt(1);
			}
			...
			c2.rs=c2.st.executeQuery("select s.asn from subdetails s, basic b where s.state like '"+state+"%' and b.subnos not in ('NA') and s.asn=b.asn order by s.state, s.fname, s.lname");
			while(c2.rs.next())
			{
			...
			//c3.rs=c3.st.executeQuery("select district from despcode where dno="+dno);
			//c3.rs.next();
			for(i=0;i<x;i++)
			{
				if(i<present)
				...
				else if(i%(linesPerPage/2)==1)
				{
					...
					textLines[i][6]="";
					textLines[i][7]="";
					//textLines[i][7]=" FOR STATE  "+state.toUpperCase();
					textLines[i][8]="";
					textLines[i][9]="";
					textLines[i][10]="";
					textLines[i][11]="";
