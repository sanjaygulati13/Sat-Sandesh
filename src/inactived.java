...
public class inactived implements Printable, ActionListener
{
	...
	public void initLines()
	{
		try
		{
			...
			c2.rs=c2.st.executeQuery("select b.asn from basic b, subdetails s where b.asn=s.asn and b.status='Inactive' order by s.state, s.fname, s.lname");
			while(c2.rs.next())
			{
		...
		if(chk%2==1 && (end-start)!=lineHeight)
		{
			...
			for(int line=start; line<end; line+=2)
			{
				...
				if(i%(linesPerPage/2)==1)
				{
					...
					g.drawString("LIST OF SAT-SANDESH MEMBERS WHOSE PERIOD HAS ENDED  BETWEEN "+mi+"-"+yi+" & "+(month-1)+"-"+year+" (STATUS INACTIVE-DETAILED)", (int)pf.getWidth()/2-350,y);
