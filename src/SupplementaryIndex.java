...
public class SupplementaryIndex implements Printable, ActionListener
{
    ...
    {
        new SupplementaryIndex(25,6,2011);
    }
    int present=0;
    int[] pageBreak;
    int numLines=0;
    ...
    public SupplementaryIndex(int d2,int m2, int y2)
    {
    ...
    public void initLines()
    {
        try
        {
            ...
            c1.rs=c1.st.executeQuery("select count(asn) from payment where InsertionDate>='"+StartDate+"'");
            while(c1.rs.next())
            {
                x=c1.rs.getInt(1);
            }
            ...
            for(i=0;i<x;i++)
            {
                ...
                if(i==0)
                {
                    ...
                    c2.rs=c2.st.executeQuery("select p.asn from payment p, subdetails s where p.asn=s.asn and p.InsertionDate>='"+StartDate+"' order by s.state, s.fname, s.lname");
                    while(c2.rs.next())
                    {
...
//                        int AsnLength=asn.length;
//
//                        for(int l=0;l<AsnLength;l++)
//                        {
//                            System.out.println("ASN : "+asn[l]);
//                        }
