...
public class freeze implements Printable, ActionListener
{
	...
	public void initLines()
	{
		try
		{
			...
			c1.rs=c1.st.executeQuery("select count(asn) from basic where status='Freeze' ");
			while(c1.rs.next())
			{
				x=c1.rs.getInt(1);
			}
			...
			c2.rs=c2.st.executeQuery("select b.asn from basic b, payment p, subdetails s where b.asn=p.asn and b.asn=s.asn and b.status='Freeze' order by p.endy, p.endm, s.fname, s.lname ");
			while(c2.rs.next())
			{
	...
	public int print(Graphics g, PageFormat pf, int pageindex)
	{
		...
		if(chk%2==1 && (end-start)!=lineHeight)
		{
            ...
            g.drawString("List Of Members Whose Period Has Ended Before "+mf+"-"+yf+" (Status FREEZED-Summary)", (int)pf.getWidth()/2-200,y+lineHeight-3);
