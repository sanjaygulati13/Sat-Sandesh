    ...
    public void actionPerformed(ActionEvent event)
    {
        if(event.getSource() == okButton)
        {
            ...
            for(int currRcpt  = 0; currRcpt < numBooklets ; currRcpt++)
            {
                ...
                String sqlQuery = "insert into receipt_book_inventory values ( '"+seriesName+"', "+(bookNum++)+", "+numReceipts+" , "+startingNum+", "+endingNum+", '' , '' , '"+SamsUtilities.getCurrentSqlDate()+"' , '"+SamsUtilities.getCurrentSqlDate()+"' ) ";
                try
                {
