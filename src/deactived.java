...
public class deactived implements Printable, ActionListener
{
	...
	public void initLines()
	{
		try
		{
			...
			c1.rs=c1.st.executeQuery("select count(asn) from basic where status='Deactive'");
			while(c1.rs.next())
			{
				x=c1.rs.getInt(1);
			}
			...
			c2.rs=c2.st.executeQuery("select b.asn from basic b, subdetails s where b.asn=s.asn and b.status='Deactive' order by s.state, s.fname, s.lname");
			while(c2.rs.next())
			{
		...
		if(chk%2==1 && (end-start)!=lineHeight)
		{
			...
			for(int line=start; line<end; line+=2)
			{
				if(i%(linesPerPage/2)==1)
				{
					...
					g.drawString("LIST OF SAT-SANDESH MEMBERS WHOSE PERIOD HAS ENDED  B/W "+(month-1)+"-"+(year-1)+" & "+(md-1)+"-"+yd+" (STATUS DEACTIVE-DETAILED)", (int)pf.getWidth()/2-350,y);
