...
public class SatSandeshFreezeDetailedList extends SamsLandscapePrintingUtil implements ActionListener
{
    public static void main(String args[])
    {
        new SatSandeshFreezeDetailedList();
    }
    int present=0;
    //int[] pageBreak;
    //int numLines=0;
    ...
    public SatSandeshFreezeDetailedList()
    {
    ...
    */
    ...
    void initAsnSet()
    {
        ...
        try{
            String query = "select count(asn) from subscribers_primary_details where membership_status='Freeze' and subscription_code not in ('NA')";
            dbConnection.rs=dbConnection.st.executeQuery(query);
            if(dbConnection.rs.next()) x = dbConnection.rs.getInt(1);
            
            asn=new int[x];            
            query = "select asn from subscribers_primary_details where membership_status='Freeze' and subscription_code not in ('NA')  order by state, first_name, last_name";
            ...
            String header_ = "Detailed List of Freeze Sat Sandesh Members Whose Period Has Ended Before "+mf+"-"+yf;
            setHeader(header_);
