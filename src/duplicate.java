...
public class duplicate extends JFrame implements ActionListener
{
    ...
    Container con;
    JButton b1,b2,b3, b4;
    Object col[]={"ASN","SUB CODE","SUB NO"};
    JComboBox subt;
    ...
    duplicate()
    {
        ...
        try
        {
            ...
            c1.rs=c1.st.executeQuery("select ANY_VALUE(asn),subscription_code from subscribers_primary_details where subscription_code not in ('NA') group by subscription_number, subscription_code having count(subscription_number)>1");
            while(c1.rs.next())
            {
                i++;
            }
            ...
            else
            {
                ...
                c2.rs=c2.st.executeQuery("select ANY_VALUE(asn),subscription_code, subscription_number from subscribers_primary_details where subscription_code not in ('NA') group by subscription_number, subscription_code having count(subscription_number)>1 ");
