...
public class alldno  extends SamsLandscapePrintingUtil implements ActionListener
{
    ...
    public alldno(int dno1)
    {
        ...
        f= new JFrame("Print List for D#"+dno);
        try
        {
    ...
    c1.rs=c1.st.executeQuery("select count(asn) from subscribers_primary_details where bulk_despatch_code="+dno+" and subscription_code not in ('NA')");
    while(c1.rs.next())
    {
    x=c1.rs.getInt(1);
    }
    ...
    c2.rs=c2.st.executeQuery("select asn from subscribers_primary_details where bulk_despatch_code ="+dno+"  and subscription_code not in ('NA') order by state, first_name, last_name");
    while(c2.rs.next())
    {
    ...
    */
    ...
    void initAsnSet()
    {
        ...
        try{
            String query = "select count(asn) from subscribers_primary_details where bulk_despatch_code="+dno+" and subscription_code not in ('NA')";
            dbConnection.rs=dbConnection.st.executeQuery(query);
            if(dbConnection.rs.next()) x = dbConnection.rs.getInt(1);
            
            asn=new int[x];
            query = "select asn from subscribers_primary_details where bulk_despatch_code ="+dno+"  and subscription_code not in ('NA') order by state, first_name, last_name";
            ...
            String distributionType = (distributionTypeText.equals("By Post"))?"DP":"DH";
            String header_ = "List of all Sat Sandesh Members Serviced by D# "+dno+" ( "+dist+" , "+stat+" ) ("+distributionType+")";
            setHeader(header_);
