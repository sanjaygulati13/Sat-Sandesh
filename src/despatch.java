...
public class despatch implements Printable, ActionListener {

   

    public static void main(String args[]) {
        new despatch("DL", 10001, 10800, "2010", "DELHI/NEW DELHI");
    }
    int[] pageBreak;
    String[] dist_type;
    String states="";
    ...
    public despatch(String subno, int start1, int end1, String header1, String footer1) {
        subnos = subno;
        end = end1;
        start = start1;
        header += header1;
        ...
        try {
            ...
            for (i = 0; i < x; i++) {
                ...
                if (i > mark) {
                    ...
                    c3.rs = c3.st.executeQuery("select b.subnos, b.subno, b.rcpt, o.ret, b.dno, b.dist from basic b, otherdet o where b.asn=" + asn[i] + " and o.asn=" + asn[i]);
            ...
            for (i = 0; i < x; i++) {
                if (asn[i] > 0) {
                    if (i % (linesPerPage / 2) > 2) {
                        c4.rs = c4.st.executeQuery("select fname, lname, state from subdetails where asn=" + asn[i]);
                        c4.rs.next();
