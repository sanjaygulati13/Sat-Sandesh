...
public class returnlist implements Printable, ActionListener
{
	...
	public void initLines()
	{
		try
		{
			...
			c1.rs=c1.st.executeQuery("select count(asn) from basic where status='STOPPED' ");
			while(c1.rs.next())
			{
				x=c1.rs.getInt(1);
			}
			...
			c2.rs=c2.st.executeQuery("select b.asn from basic b, payment p, subdetails s where b.asn=p.asn and b.asn=s.asn and b.status='STOPPED' order by p.endy, p.endm, s.fname, s.lname ");
			while(c2.rs.next())
			{
			...
			for(i=0;i<x;i++)
			{
				...
				else if(i%linesPerPage>2)
				{
					...
					c3.rs=c3.st.executeQuery("select b.asn, b.subnos, b.subno, s.fname, s.lname, s.dist, s.state, o.ret ,o.rec_date, o.post , o.oth_remarks from basic b, otherdet o, subdetails s where o.asn="+asn[i]+" and s.asn="+asn[i]+" and b.asn="+asn[i]);
					c3.rs.next();
