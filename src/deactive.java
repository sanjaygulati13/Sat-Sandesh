...
public class deactive implements Printable, ActionListener
{
	...
	public void initLines()
	{
		try
		{
			...
			c2.rs=c2.st.executeQuery("select b.asn from basic b, payment p, subdetails s where b.asn=p.asn and b.asn=s.asn and b.status='Deactive' order by p.endy, p.endm, s.fname, s.lname ");
			while(c2.rs.next())
			{
	...
	public int print(Graphics g, PageFormat pf, int pageindex)
	{
		...
		if(chk%2==1 && (end-start)!=lineHeight)
		{
            ...
            g.drawString("List Of Members Whose Period Has Ended Between "+(month-1)+"-"+(year-1)+" & "+(md-1)+"-"+yd+" (Status DEACTIVE-Detailed)", (int)pf.getWidth()/2-230,y+lineHeight-3);
