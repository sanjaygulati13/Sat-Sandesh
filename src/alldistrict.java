...
public class alldistrict extends SamsLandscapePrintingUtil implements ActionListener
{
    ...
    public void initLines()
    {
        try
        {
            ...
            //c1.rs=c1.st.executeQuery("select count(s.asn) from subdetails s, basic b where s.dist like '"+dist+"%' and s.state like '"+state+"%' and b.subnos not in ('NA') and s.asn=b.asn");
            c1.rs=c1.st.executeQuery("select count(asn) from subscribers_primary_details where district like '"+dist+"%' and state like '"+state+"%' and subscription_code not in ('NA')");
            while(c1.rs.next())
            {
                x=c1.rs.getInt(1);
            }
            ...
            for(i=0;i<x;i++)
            {
                ...
                if(i==0)
                {
                    ...
                    c2.rs=c2.st.executeQuery("select asn from subscribers_primary_details where district like '"+dist+"%' and state like '"+state+"%' and subscription_code not in ('NA')  order by state, first_name, last_name");
                    while(c2.rs.next())
                    {
        ...
        if(chk%2==1 && (end-start)!=lineHeight)
        {
            ...
            for(int line=start; line<end; line+=2)
            {
                if(i%(linesPerPage/2)==1)
                {
                    ...
                    g.drawString("LIST OF SAT-SANDESH MEMBERS - "+dist.toUpperCase() +" ("+state.toUpperCase()+") ", (int)pf.getWidth()/2-200,y);
