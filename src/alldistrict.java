...
public class alldistrict implements Printable, ActionListener
{
	...
	public void initLines()
	{
		try
		{
			...
			c2.rs=c2.st.executeQuery("select s.asn from subdetails s, basic b where s.dist like '"+dist+"%' and s.state like '"+state+"%' and b.subnos not in ('NA') and s.asn=b.asn order by s.state, s.fname, s.lname");
			while(c2.rs.next())
			{
